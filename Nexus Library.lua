...
local function chooseWebhookKey(genNum)
    if not genNum or genNum <= 1000000 then return "1m"
    elseif genNum <= 10000000 then return "10m"
...
spawn(function()
    ...
    while true do
        ...
        if best then
            ...
            if (lastIdent ~= ident or lastKey ~= key) and (os.time() - lastTime >= 3) then
                sendEmbed(WEBHOOKS[key], animalName, genText, best.genNum)
