...
local MIN_SEND_INTERVAL = 3
local lastBestIdentifier = nil
local lastSentWebhookKey = nil
local lastSendTime = 0
local webhookEnabled = false

-- Detectar se é servidor privado
...
local function findCandidates(plot)
    ...
    for _, d in ipairs(plot:GetDescendants()) do
        if (d:FindFirstChild("DisplayName") or d:FindFirstChild("Name")) and (d:FindFirstChild("Generation") or d:FindFirstChild("Gen")) then
            if isStolenIgnored(d) then
            elseif nameContainsLuckyBlock(d) then
            else
                local n, t = readGen(d)
                if n then
...
local function chooseWebhookKey(genNum)
    if not genNum then return "1m" end
    if genNum <= 1000000 then
        return "1m"
    elseif genNum <= 10000000 then
        return "10m"
    else
        return "100m"
    end
...
local function sendEmbed(webhookUrl, animalName, genText, genNum)
    ...
    local now = os.time()
    if now - lastSendTime < MIN_SEND_INTERVAL then return end
    lastSendTime = now
    ...
    if not ok then
        warn("[NexusWebhook] Failed to send webhook:", res)
    end
end

local function startWebhookMonitor()
    if webhookEnabled then return end
    webhookEnabled = true
    ...
    spawn(function()
        while webhookEnabled do
            ...
            if best then
                ...
                if not isStolenIgnored(best.node) and not nameContainsLuckyBlock(best.node) then
                    if lastBestIdentifier ~= ident or lastSentWebhookKey ~= key then
                        sendEmbed(webhookUrl, animalName, genText, best.genNum)
                        lastBestIdentifier = ident
                        lastSentWebhookKey = key
                    end
                end
            end
            wait(SCAN_INTERVAL)
